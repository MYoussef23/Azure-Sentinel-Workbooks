...
def get_access_token_once(resource: str) -> Tuple[Optional[str], str]:
    """
...
def ensure_access_token(resource: str, tenant_id: Optional[str] = None, scope_for_login: Optional[str] = None) -> str:
    ...
    """
    token, txt = get_access_token_once(resource)
    if token:
        return token
    ...
    if AADSTS_NEEDS_INTERACTION.search(txt):
        ...
        token, txt2 = get_access_token_once(resource)
        if token:
            return token
...
def azure_monitor_login(tenant_id: Optional[str] = None) -> Optional[str]:
    ...
    try:
        ...
        print(f"Session token for {mgmt_resource} is valid.")
    except Exception as e:
    ...
    if current_sub:
        if not prompt_change_subscription(current_sub):
            ...
            new_sub = list_and_select_subscription(current_account or "")
            if new_sub:
                current_sub = new_sub
        ...
        current_sub = list_and_select_subscription(current_account)
        if not current_sub:
...
def _query_log_analytics(workspace_id: str, query: str, timespan: str, verify_tls: bool, tenant_id: Optional[str] = None) -> dict:
    """
